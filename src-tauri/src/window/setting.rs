...
pub fn show_setting_window(app: &AppHandle) -> Result<(), tauri::Error> {
    ...
    let _ = app.run_on_main_thread(move || {
        let _ = create_setting_window(&app_clone);
    });
    Ok(())
}

/// Creates the setting window with saved state restoration.
fn create_setting_window(app: &AppHandle) -> Result<(), tauri::Error> {
    let app_handle = app.clone();
    let config_file = "setting-window-state.json";
    let state = WindowState::load(&app_handle, config_file);
    let main_url = WebviewUrl::App("#/setting".into());

    let mut builder = WebviewWindowBuilder::new(app, "setting", main_url)
        .title("Setting")
    ...
    if let (Some(x), Some(y)) = (state.x, state.y) {
        ...
        builder = builder.center();
        info!("No saved setting window position. Centering window.");
    }
