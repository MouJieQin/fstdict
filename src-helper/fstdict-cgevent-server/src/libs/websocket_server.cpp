...
WebSocketServer::WebSocketServer(size_t max_clients)
    : max_clients(max_clients), client_connected(max_clients, false), client_mutexes(max_clients), event_queue(),
      events_map_clients(getEnumNames(EventType{}).size()), clients_map_events(max_clients)
{
...
void WebSocketServer::push_event_json(const json &j)
{
    event_queue.push(j);
}
...
string WebSocketServer::base64_encode(const unsigned char *buffer, size_t length)
{
    BIO *bio, *b64;
    BUF_MEM *buf;
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (type == "register_request")
            {
                if (!j.contains("data"))
                {
                    LOG_WARN("[WebSocket] [{}] 解析 JSON 消息失败: 缺少 data 字段", client);
                }
                else
                {
                    ...
                    {
                        LOG_WARN("[WebSocket] [{}] 解析 JSON 消息失败: 缺少 event 字段", client);
                    }
                    else
                    {
                        ...
                        if (!type)
                        {
                            LOG_WARN("[WebSocket] [{}] 解析 JSON 消息失败: 未知 event: {}", client, event);
                        }
                        else
                        {
            ...
            {
                LOG_WARN("[WebSocket] [{}] 解析 JSON 消息失败: 未知 type: {}", client, type);
            }
        }
...
void WebSocketServer::handle_event_queue()
{
    while (true)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(events_map_mutex);
            auto event_type = EventTypeEnum::fromString(std::string(event_data["data"]["type"]));
...
void WebSocketServer::start_websocket_server()
{
    ...
    while (true)
    {
        ...
        if (client_fd >= 0)
        {
            ...
            {
                close(client_fd);
                LOG_WARN("[WebSocket] [{}] accept failed: 客户端数量[{}] 已达上限, 已拒绝连接", client_fd, max_clients);
            }
            else
            {
                LOG_INFO("[WebSocket] accept success: [{}]", client_fd);
                thread(&WebSocketServer::handle_ws_client, &ws_server, client_fd).detach();
            }
        }
        else
        {
