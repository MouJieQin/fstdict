...
{
public:
    ShortcutRunner(const dblisp::recursive_map &shortcutConfig, CGEventHandler &cgEventHandlerRef, std::unordered_map<CGKeyCode, Time> &last_time) : config(shortcutConfig), double_typing_interval(500000), shortcuts_map(16, nullptr), shortcut_enabled(true), selection_monitor_enabled(false), selection_monitor_callback_enabled(false), selection_monitor_callback_cmd(""), cgEventHandlerRef(cgEventHandlerRef), last_time(last_time)
    {
...
private:
    ...
    std::vector<std::shared_ptr<std::unordered_map<std::string, dblisp::recursive_map>>> shortcuts_map; // 内部快捷键映射（按需设计数据结构）
